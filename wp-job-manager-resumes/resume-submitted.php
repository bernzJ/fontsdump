		...
		if ( resume_manager_user_can_view_resume( $resume->ID ) ) {
			...
			echo wp_kses_post(
				sprintf(
					...
					__( 'Your resume has been submitted successfully.', 'wp-job-manager-resumes' ),
					esc_url( get_permalink( $resume->ID ) )
				)
			);
			echo '</p>';
			$resume_link = get_permalink( $resume->ID); ?>
				<div class="submitted-btn-set">
						<a class="btn btn-main btn-effect" href="/submit-vehicle/"> Add another vehicle </a>
						<a class="btn btn-main btn-effect"href="<?=$resume_link; ?>"> View my vehicle </a>
				</div>
		<?php } else {
			echo '<p class="resume-submitted">';
