	...
	<?php if ( resume_manager_user_can_post_resume() ) : ?>
		...
		<?php foreach ( $resume_fields as $key => $field ) : ?>
			<fieldset class="fieldset-<?php esc_attr_e( $key ); ?>">
				<label for="<?php esc_attr_e( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_resume_form_required_label', $field['required'] ? '' : ' <small>' . esc_html__( '(optional)', 'cariera' ) . '</small>', $field ); ?></label>
				<div class="field">
		...
		<div class="cariera-listing-submission">
			...
			<input type="submit" name="submit_resume" class="button" value="<?php esc_attr_e('Submit vehicle'); ?>" />
        </div>
